...
function cfn_getComCodeList(codeList){
	$.doPost({
		url 	: "/com/getCodeList",
		data	: {
			code_list : codeList
		...
		success : function(result){
			for(code in result){
				_comCode[code] = result[code];
			}
		}
	})
}

function cfn_setCombo(comboList){
	comboList.forEach(combo => {
		combo.data.forEach(data => {
			let tagStr = '<option value="'+data.value+'">'+data.text+'</option>';
			$('#'+combo.id).append(tagStr);
		});
	});
}

function cfn_setComboAllOption(combo,optionText){
	let list = [];
	let obj = {};
	obj.value='';
...
function cfn_setComboOption(combo){
	let list = [];
	combo.forEach(data=>{
		list.push(data);
	})
